  ...
  Repeat
    ...
    Select WaitWindowEvent()
      ...
      CreateFile(0, "hosts.dat")
       Result = CountGadgetItems(#Hosts_List)
        COLFLAG=countlisticoncolumn(#Hosts_List,3,"")
  For lin = 0 To Result - 1;number of lines
     For col = 0 To COLFLAG - 1;number of columns
      plink$ = GetGadgetItemText(#Hosts_List,lin,col);get that LOCATION and put it in plink$
      plink$ = plink$+",";add a comma (this is a CSV file)
        plonk$ = plonk$ + plink$;add plink$ to plonk$
         ;plink$ = "";kill value in plink$
          If col=COLFLAG-1;EOL reached
            cut=Len(plonk$) - 1
             plonk$ = Left(plonk$,cut);cut off the last ","
          EndIf
       Next
       WriteStringN(0, plonk$);write the entire plonk$
...
Protected myostype.s
  ...
  If objWMIService
    colOSType = objWMIService\CreateEnumeration("ExecQuery('Select * from Win32_OperatingSystem')");Win32_PhysicalMemory
   If colOSType
...
Procedure SaveFile()
Repeat
 items=CountGadgetItems(#Hosts_List)
  OpenFile(0,"hosts.dat")
   For x=0 To items
    myname.s=GetGadgetItemText(#Hosts_List,x)
     mydescription.s=GetGadgetItemText(#Hosts_List,x,1)
      whyme.s=""
       whyme.s + myname + Chr(44) + mydescription
      If whyme.s<>""
       If whyme.s<>","
        If whyme.s<>"name"
         If whyme.s<>"description"
          WriteStringN(0,whyme,#PB_Ascii)
         EndIf
        EndIf
       EndIf
      EndIf
